(function(){
  ...
  function badge(code, title){
    ...
    const known = [
      "vf-code-T","vf-code-N","vf-code-W","vf-code-F","vf-code-FF","vf-code-A",
      "vf-code-S","vf-code-SB","vf-code-K","vf-code-B","vf-code-H","vf-code-L",
      "vf-code-O","vf-code-M"
    ].includes(cls);
  ...
  function groupSections(employees){
    return Array.from(new Set((employees || []).map(e => e.section || e.area).filter(Boolean))).sort((a,b)=>a.localeCompare(b,"da"));
  }
  ...
  function summarize(days, year, legend){
    ...
    return Array.from(counts.entries())
      ...
      .map(([code,count]) => ({ code, count, text: (legend || {})[code] || code }));
  }

  function monthDaysFor(employee, year, month){
    return (employee?.days || []).filter(d => Number(d.year) === Number(year) && Number(d.month) === Number(month));
  }

  function upcomingFor(employee, today, limit){
    return (employee?.days || []).filter(d => d.date >= today).slice(0, limit);
  }

  function currentFor(employee, today){
    return (employee?.days || []).find(d => d.date === today) || null;
  }
  ...
  function renderEmployeeButtons(data, state, selectedName){
    ...
    return rows.map(e => `
      <button class="vf-employee-btn ${e.name === selectedName ? "active" : ""}" data-vf-employee="${esc(e.name)}">
        <div class="vf-employee-name">${esc(e.name)}</div>
        <div class="vf-employee-area">${esc(e.area)}${e.section && e.section !== e.area ? ` · ${esc(e.section)}` : ""}</div>
      </button>
    `).join("");
  }
        ...
        <aside class="vf-side">
          <div class="vf-toolbar">
            <select id="vfSection">
              ...
              ${sections.map(a => `<option value="${esc(a)}" ${state.section === a ? "selected" : ""}>${esc(a)}</option>`).join("")}
            </select>
        ...
        <main class="vf-main">
          ...
          <div class="vf-legend">
            ${Object.entries(data.legend || {}).map(([c,t]) => `
              <div class="vf-summary-item">${badge(c,t)} <span>${esc(t)}</span></div>
            `).join("")}
          </div>
        </main>
      </div>
  ...
  function wire(container, data, state, refreshFn){
    const rerender = () => {
      ...
      wire(container, data, state, refreshFn);
    };
    ...
    container.querySelector("#vfYear")?.addEventListener("change", e => {
    ...
    container.querySelector("#vfSearch")?.addEventListener("input", e => {
