...
async function getDataverseToken() {
  ...
  if (!r.ok || !data.access_token) {
    throw new Error(
      `token_error ${r.status}: ${data.error_description || data.error || JSON.stringify(data)}`
    );
  }
  ...
  try {
    ...
    if (!indhold) {
      return json(context, 400, {
        ...
        message: "Indhold må ikke være tomt."
      });
    }
    ...
    if (!new Set(["Nyhed", "Tip"]).has(valg)) {
      return json(context, 400, {
        error: "invalid_type",
        message: "Type skal være Nyhed eller Tip."
      });
    }
